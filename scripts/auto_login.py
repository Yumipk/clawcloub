            ...
            if not ok:
                ...
                for s in reversed(self.shots):
                    if any(keyword in s for keyword in keywords):
                        post_login_shots.append(s)
                    if len(post_login_shots) >= 3:  # 最多3张
                        break
                
                if post_login_shots:
                    # 反转回来，按时间顺序发送
                    post_login_shots.reverse()
                    for s in post_login_shots:
                        self.tg.photo(s, os.path.basename(s))
                else:
                    # 如果没有找到特定关键词的截图，发送最后3张
                    for s in self.shots[-3:]:
                        self.tg.photo(s, os.path.basename(s))
        ...
        with sync_playwright() as p:
            ...
            try:
               ...
               # 2. 点击 GitHub
                ...
                # 重定向成功后立即截图（重要！）
                self.shot(page, "登录成功_重定向后")
                self.log(f"重定向成功，当前页面: {page.url}", "SUCCESS")
                self.log(f"页面标题: {page.title()}", "INFO")
